...
import { MdSchool, MdPeople, MdTrendingUp, MdGroup, MdAccessTime, MdStar, MdWarning } from 'react-icons/md';
  ...
  async function loadAdminStats() {
    ...
    try {
      ...
      const courseProgress = await Promise.all(activeCourses.map(async course => {
        ...
        if (!activeL.length) return { ...course, avgProgress: 0, completions: 0 };
      ...
      // Alumnos inactivos (sin actividad en 14 días)
      ...
      setStats({
        ...
        topStudents,
        weeklyCompletions: recentProgress?.length || 0,
      });
    } catch (err) {
      console.error(err);
    } finally {
                ...
                <Card key={course.id} hover onClick={() => router.push('/courses')}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-1" style={{ color: '#5a5a70' }}>{course.description || 'Sin descripción'}</p>
                    </div>
                  </div>
                </Card>
              ))}
      ...
      {currentRating && (
        <Modal isOpen={showRatingModal} onClose={handleSkip} title="Evaluación de clase">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-3">
                <button onClick={() => setAttended(true)} className="flex-1 py-3 rounded-xl text-sm font-medium transition-all"
                  style={{ background: attended === true ? '#4ade8020' : '#0f0f13', border: `1px solid ${attended === true ? '#4ade80' : '#333344'}`, color: attended === true ? '#4ade80' : '#9090a8' }}>✓ Sí asistí</button>
                <button onClick={() => { setAttended(false); setRatingValue(0); }} className="flex-1 py-3 rounded-xl text-sm font-medium transition-all"
                  style={{ background: attended === false ? '#f75c6a20' : '#0f0f13', border: `1px solid ${attended === false ? '#f75c6a' : '#333344'}`, color: attended === false ? '#f75c6a' : '#9090a8' }}>✗ No asistí</button>
              </div>
            </div>
            {attended === true && <div className="flex flex-col gap-2"><p className="text-sm text-center" style={{ color: '#9090a8' }}>¿Cómo estuvo la clase?</p><StarRating value={ratingValue} onChange={setRatingValue} /></div>}
            ...
            {pendingRatings.length > 1 && <p className="text-xs text-center" style={{ color: '#5a5a70' }}>{pendingRatings.length - 1} evento{pendingRatings.length > 2 ? 's' : ''} más por evaluar</p>}
            <div className="flex gap-3">
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm" style={{ color: '#5a5a70' }}>Panel de administración</p>
      </div>

      {loadingStats ? (
        <div className="flex items-center justify-center py-20">
          <p style={{ color: '#5a5a70' }}>Cargando estadísticas...</p>
        </div>
      ) : stats && (
        <div className="flex flex-col gap-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard icon={<MdStar />} label="Lecciones completadas" value={stats.weeklyCompletions}
              color="#f75c6a" sub="Últimos 7 días" />
          </div>
                  ...
                  <Card key={course.id}>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs mt-1" style={{ color: '#5a5a70' }}>{course.completions} lecciones completadas · {course.totalLessons} lecciones totales</p>
                      </div>
                    </div>
                  </Card>
                ))}
          ...
          {/* Top alumnos y alumnos inactivos */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card>
                <div className="flex flex-col gap-3">
                  {stats.topStudents.length === 0
                    ? <p className="text-center py-4 text-sm" style={{ color: '#5a5a70' }}>Sin actividad aún.</p>
                    : stats.topStudents.map((s, i) => (
                      <div key={s.id} className="flex items-center gap-3">
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0"
                          style={{ background: i === 0 ? '#fbbf2420' : '#2a2a38', color: i === 0 ? '#fbbf24' : '#5a5a70' }}>
                          {i + 1}
                        </div>
            ...
            <div>
              <h2 className="font-display font-semibold text-white text-lg mb-4">
                <span style={{ color: '#f75c6a' }}>⚠️</span> Alumnos inactivos <span className="text-sm font-normal" style={{ color: '#5a5a70' }}>(+14 días)</span>
              </h2>
              <Card>
                <div className="flex flex-col gap-3">
                  {stats.inactiveStudents.length === 0
                    ? <p className="text-center py-4 text-sm" style={{ color: '#4ade80' }}>✓ Todos los alumnos están activos</p>
